...
namespace KecCak512Sha3
{
    public class Program
    {
        ...
        /*const */
        static private Byte[] w_array = { 1, 2, 4, 8, 16, 32, 64 }; //b / 25; 1600 / 25 for 512
        /*const */
        ...
        static private Byte[] Keccak(UInt16 rate, UInt16 capacity, List<Byte> Message)
        {
            ...
            UInt16 n = (UInt16)Math.Ceiling(((float)Message.Count / (float)min));
            int messageFullCount = 0;
            ...
            {
                messageFullCount = (n * min + (n - 1));
            }

            UInt32 delta = (UInt32)(messageFullCount - Message.Count);

            //Check size of message.
            if (messageFullCount > UInt16.MaxValue - 1)
                throw (new Exception("Message might be too large"));

            while (delta > 0)  // Дополнение нулевыми битами до тех пор, пока длина не будет кратна Rate
            {
                Message.Add(0x00);
                delta--;
            }
            ...
            Message.Add(0x80); //Обозначает конец сообщения
            /*const*/
            ...
            for (xF = 0; xF < Message.Count; xF++)
            {
                ...
                {
                    P[xd, i + j] = ReverseEightBytesAndToUInt64(
            ...
            for (xF = 0; xF < size; xF++)
            {
                for (i = 0; i < 5; i++)
                    for (j = 0; j < 5; j++)
                        ...
                        {
                            S[i, j] = S[i, j] ^ P[xF, i + j * 5];
                        }
                S = Keccak_f(S);

                Console.WriteLine($"{xF + 1} step:");
                foreach (var x in S)
                {
                    Console.Write(ByteArrayToString(FromUInt64ToReverseEightBytes(x)).ToString());
                }
                Console.WriteLine();
            }
                    ...
                    for (j = 0; j < 5; j++)
                        if ((5 * i + j) < (rate / w_array[InstanceNumber]))
                        {
                            ...
                            else
                            {
                                ...
                                Console.WriteLine(ByteArrayToString(FromUInt64ToReverseEightBytes(S[j, i]).ToArray()));
        ...
        static void Main()
        {
            ...
            do
            {
                Console.Write("Введите сообщение для зашифрования: ");
        ...
        {
            List<byte> byteMessage = new List<byte>(str.Length);

            foreach (char ch in str)
            {
                byteMessage.Add((byte)ch);
            }
        ...
        static public string ByteArrayToString(byte[] b)
        {
            ...
            for (int i = 0; i < Math.Min(b.Length, Int32.MaxValue - 1); i++)
